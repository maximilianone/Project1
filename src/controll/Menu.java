...
public class Menu implements Patterns{
    ...
    public void run() {
        menu = true;
        fileController.loadFile(homeAppliances);
        while (menu) {
            menuView.showMenu();
    ...
    private void findAppliance(){
        Range sizeBorders = inputBorders(MenuView.REQUEST_SIZE);
        Range powerBorders = inputBorders(MenuView.REQUEST_POWER);
        menuView.showAppliances((filterAppliances.findAppliance(sizeBorders,powerBorders,homeAppliances)));
    }

    private Range inputBorders(String request){
        menuView.displayMessage(request);
        Range borders = new Range();
        borders.setLeft(Double.parseDouble(inputValue(MenuView.REQUEST_LEFT,DOUBLE_PATTERN)));
        while (!borders.setRight(Double.parseDouble(inputValue(MenuView.REQUEST_RIGHT,DOUBLE_PATTERN)))){
